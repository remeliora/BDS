...
def hits_algorithm(DG, max_iter=100, tolerance=1e-6):
    """Ручная реализация алгоритма HITS без зависимостей"""
    nodes = list(DG.nodes())
    n = len(nodes)
    ...
    for _ in range(max_iter):
        ...
        # Обновляем authority scores
        for node in nodes:
            for predecessor in DG.predecessors(node):
                new_authority[node] += hub[predecessor]

        # Обновляем hub scores
        for node in nodes:
            for successor in DG.successors(node):
                new_hub[node] += new_authority[successor]

        # Нормализация
        auth_norm = math.sqrt(sum(a ** 2 for a in new_authority.values()))
        hub_norm = math.sqrt(sum(h ** 2 for h in new_hub.values()))

        if auth_norm > 0:
            new_authority = {node: score / auth_norm for node, score in new_authority.items()}
        if hub_norm > 0:
            new_hub = {node: score / hub_norm for node, score in new_hub.items()}

        # Проверка сходимости
        auth_diff = sum(abs(new_authority[node] - authority[node]) for node in nodes)
        hub_diff = sum(abs(new_hub[node] - hub[node]) for node in nodes)
...
def visualize_authority_graph(pos):
    ...
    # Рассчитываем авторитетность через ручную реализацию HITS
    authorities, _ = hits_algorithm(DG)
